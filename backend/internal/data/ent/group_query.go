...
func (_q *GroupQuery) QueryAiCaptureSessions() *AICaptureSessionQuery {
...
func (_q *GroupQuery) WithAiCaptureSessions(opts ...func(*AICaptureSessionQuery)) *GroupQuery {
	query := (&AICaptureSessionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withAiCaptureSessions; query != nil {
		if err := _q.loadAiCaptureSessions(ctx, query, nodes,
			func(n *Group) { n.Edges.AiCaptureSessions = []*AICaptureSession{} },
			func(n *Group, e *AICaptureSession) { n.Edges.AiCaptureSessions = append(n.Edges.AiCaptureSessions, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *GroupQuery) loadAiCaptureSessions(ctx context.Context, query *AICaptureSessionQuery, nodes []*Group, init func(*Group), assign func(*Group, *AICaptureSession)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(aicapturesession.FieldGroupID)
	}
	query.Where(predicate.AICaptureSession(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.AiCaptureSessionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
